...
var Mediator = function(virgilio) {
    ...
    getMessage(this.eventEmitter, virgilio);
};

//getMessage polls the stream for new messages whenever the I/O queue is clear.
function getMessage (emitter, virgilio) {
    var message = virgilio.stream$.read();
    setImmediate(getMessage, emitter, virgilio);
    if (!message) { return; }
    emitter.emit(message.topic, message.args, message.replyTopic);
}
