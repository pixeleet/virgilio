...
var Virgilio = function Virgilio(options) {
    ...
    //In this way we can require modules during initialization and then access to
    //them without requiring them in every submodule.
    this.require$ = {};
};
...
Virgilio.prototype._createAction$ = function _createAction$(name, handler) {
    ...
    action = this._bindPromise$(action);
...
//Bind the promise, returned by the action, to 'this'
Virgilio.prototype._bindPromise$ = function _bindPromise$(action) {
    var virgilio = this;
    return function () { return action.apply(null, arguments).bind(virgilio) };
}
