...
var Concordia = function Concordia(options) {
    ...
    this.util$.validateArg('Concordia', 'options', options, 'object');
...
Concordia.prototype.defineAction$ = function defineAction$(path, handler) {
    this.util$.validateArg('defineAction$', 'path', path, 'string');
...
Concordia.prototype.extend$ = function extend$(methodName, replacementMethod) {
    this.util$.validateArg('extend$', 'methodName', methodName, 'string');
...
//Add custom errors
Concordia.prototype.registerError$ = function registerError$(name, init) {
    if (typeof name === 'function') {
        init = name;
        name = init.name;
    }
    this.util$.validateArg('registerError$', 'name', name, 'string');
    this.util$.validateArg('registerError$', 'init', init,
                           ['function', 'undefined']);
    if (this.baseConcordia$[name]) {
        throw new this.DuplicateErrorRegistrationError(name);
    }
    this.baseConcordia$[name] = util.createCustomError(name, init);
};
